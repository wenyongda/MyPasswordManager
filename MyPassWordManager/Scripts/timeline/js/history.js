function history() {
    ...
    $(function () {
        function e() {
            //c = $(".timeline li").eq(d).find(".year").html();
        ...
        $(".timeline .liwrap").mouseover(function () {
            $(".timeline li").removeClass("thiscur");
            $(this).parent().addClass("thiscur")
        });
        var b = 0, f = $(".timelinelist li").length, d = 0, a = !0, c;
        (function () {
            var a = $(window).height();
            890 <= a && (row = 9);
            800 <= a && 890 > a && (row = 8);
        ...
        $(".arrowdown").click(function () {
            a && parseInt($(".timelinelist li:first").css("marginTop")) > -110 * h && (d++, b--, a = !1, $(".timelinelist li").eq(d - 1).liOut(), $(".timelinelist li").eq(d + row - 1).liIn(), $(".timelinelist li:first").animate({ marginTop: 110 * b }, 600, "easeInOutQuad", function () {
        ...
        $(".arrowup").click(function () {
            a && 0 != parseInt($(".timelinelist li:first").css("marginTop")) && (b++, d--, a = !1, $(".timelinelist li").eq(d).liIn(), $(".timelinelist li").eq(d + row).liOut(), $(".timelinelist li:first").animate({ marginTop: 110 * b }, 600, "easeInOutQuad", function () {
                a = !0; $(".arrowup").css("opacity", 1);
                $(".arrowup").removeClass("arrow_active")
            }), e()
		)
        });
        $(".timelinelist").mousewheel(function (b, c) {
            Math.abs(c);
            if (a) {
                var d = parseInt($(".timelinelist li:first").css("marginTop"));
        ...
        $(".timeblock").attr("thisyear", c);
        $(".timelinelist li:gt(" + (row - 1) + ")").find(".lileft").css({ left: "-400px" });
        $(".timelinelist li:gt(" + (row - 1) + ")").find(".liright").css({ right: "-700px" });
